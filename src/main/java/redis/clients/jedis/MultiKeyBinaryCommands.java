...
public interface MultiKeyBinaryCommands {
  ...
  String xpush(final byte[] key,final byte[] value);
  
  String xreset(final byte[] key,final byte[] value);
  
  Long smembersmulti(byte[]... keys);
  
  Long zoff(final byte[] key, final double score, final byte[] member);
  
  Long zoff(final byte[] key, final Map<byte[], Double> scoreMembers);
  
  String zreset(final byte[] key,final byte[] value);
  
  Long hgetallmulti(byte[]... keys);
}
