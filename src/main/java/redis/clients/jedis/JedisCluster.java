...
public class JedisCluster extends BinaryJedisCluster implements JedisClusterCommands,
  ...
  public List<Object> multiWithOneKey(final String key, final MultiOperation[] mop) {
      ...
      public List<Object> execute(Jedis connection) {
    	  try {
			Transaction t = connection.multi();
			  for(MultiOperation mo:mop){
				  mo.operate(key,connection.getClient());
			  }
			  return t.exec(mop.length+1);
		} catch (Exception e) {
			throw e;		
		}
      }
