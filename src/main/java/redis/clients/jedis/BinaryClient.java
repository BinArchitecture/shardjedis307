...
public class BinaryClient extends Connection {
  ...
  public void zoffBinary(final byte[] key, final Map<byte[], Double> scoreMembers) {
...
public void subscribe(final byte[]... channels) {
  ...
  public void xpush(final byte[] key, final byte[] value) {
	  sendCommand(XPUSH, key,value);
  }
  
  public void xreset(final byte[] key, final byte[] value) {
	  sendCommand(XRESET, key,value);
  }
  
  public void smembersmulti(final byte[]... keys) {
	  sendCommand(SMEMBERSMULTI, keys);
  }
  
  public void zoff(final byte[] key, final double score, final byte[] member) {
	    sendCommand(ZOFF, key, toByteArray(score), member);
  }
  
  public void zreset(final byte[] key, final byte[] value) {
	  sendCommand(ZRESET, key,value);
  }
  
  public void hgetallmulti(final byte[]... keys) {
	  sendCommand(HGETALLMULTI, keys);
  }
}
